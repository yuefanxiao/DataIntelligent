...
- 验收环境与生产形态存在 schema 差异（public vs bss 域 schema 前缀）：
  v1 执行路径只认 public 的如实反映；执行路径支持 schema 前缀时 fixture
  换前缀 + provisioning 覆盖即可，无需改用例。
- 语义数据变更（指标/概念/枚举改动）会直接作用于验收：主用例的
  dry-run 全文断言与语义用例的 FQN 断言会把「口径改动」暴露为用例失败
  ——验收与语义数据同频，这是 golden 语料复用的前提。
- 新增退款指标会破坏 neg-005 的「退款域无指标」假设：换域即可（隔离
  策略已文档化，README/清单有指引）。
- fixture 数据是确定性伪造数据（相对 now()），与生产真实数据无关：
  数字一致（三件套 a）对照的是「网关 = psql 同库同数据」，不是生产值。
